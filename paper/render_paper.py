...
<title>ThetaMem: Signed Multiplicative Lifts for Fixed-State Sequence Memory</title>
<style>{CSS}</style>
<script>
window.MathJax = {{
  tex: {{
...
def _stamp_pdf(raw_pdf: Path, output: Path) -> None:
    ...
    for number, page in enumerate(reader.pages, start=1):
        ...
        stamp.drawString(18 * mm, 7.7 * mm, "The ThetaMem Project")
    ...
    writer.add_metadata(
        {
            ...
            "/Subject": "Signed multiplicative lifts for fixed-state sequence memory",
            "/Author": "The ThetaMem Project",
            "/Creator": "paper/render_paper.py",
            "/Producer": f"pypdf {__import__('pypdf').__version__}",
            "/Keywords": "ThetaMem, associative memory, linear attention, signed features, multiplicative lift, tensor state, delta rule",
...
def render(source: Path, output: Path, chrome: Path, keep_html: Path | None) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="thetamem-paper-") as directory:
        work = Path(directory)
        ...
        if figures.is_dir():
            # The HTML is printed from this temporary directory, so
            # repository-relative image paths must resolve beside it.
